...
describe('cachify', () => {
  describe('with string key', () => {
    it('and string value', async (t) => {
      ...
      const keyv = new Keyv();
      const get = t.mock.method(keyv, 'get');
      const set = t.mock.method(keyv, 'set');
      const cachedFunc = cachify(keyv, func);
      ...
      // set() isn't called for a cache hit
      assert.equal(set.mock.calls.length, 1);
      ...
      assert.equal(set.mock.calls.length, 2);
    });
    it('and normalized key', async (t) => {
      ...
      const get = t.mock.method(keyv, 'get');
      const set = t.mock.method(keyv, 'set');
      const cachedFunc = cachify(keyv, func, normalizeKey);
